    ...
    public function stage_1_change_status()
    {
        ...
        if ($status == 'Lodged') {
            ...
            if ($mail_check == 1 && $mail_check_2 == 1) {
                $admin_status = [
                    'status' => $status,
                    'expiry_date' => date('Y-m-d H:i:s', strtotime('+60 days', strtotime(date('Y-m-d H:i:s')))),
            ...
            // new add on 04-04-2023 by vishal as par clint call
            $to =   env('DIPREET_EMAIL');
            $check_2 = send_email(env('SERVER_EMAIL'), env('SERVER_FROM_EMAIL'), $to, $subject, $message);
            if ($check_2 == 1) {
                $mail_check_2 = 1;
            }
            ...
            $mail_check_2 = 0;
            // s1_declined_admin
            $mail_temp_9 = $this->mail_template_model->asObject()->where(['id' => '9'])->first();
    ...
    public function stage_2_change_status()
    {
        ...
        if ($status == 'Lodged') {
            ...
            if ($client_type == "Agent") {
                ...
                $mail_temp_3 = $this->mail_template_model->asObject()->where(['id' => '21'])->first();
                $subject = mail_tag_replace($mail_temp_3->subject, $pointer_id);
                $message = mail_tag_replace($mail_temp_3->body, $pointer_id);
                $to = $user_mail;
                $check = send_email(env('SERVER_EMAIL'), env('SERVER_FROM_EMAIL'), $to, $subject, $message);
    ...
    // exit;
        ...
        //     exit;
            ...
            if ($s1_occupation->pathway == 'Pathway 1') {
                ...
                $qualification_file_name = 'Qualification letter - '.$name.'.'. $qualification_file_type;
      ...
      $upload_outcome_path = $target_file . '/' . $outcome_file_name;
            ...
            if ($s1_occupation->pathway == 'Pathway 1') {
                ...
                if ($client_type == "Applicant") {
                    ...
                    $mail_temp_4 = $this->mail_template_model->asObject()->where(['id' => '108'])->first();
            ...
            $reason_file_name = 'Statement_of_reason - '.$name.'.'. $reason_file_type;
...
public function stage_4_change_status(){
    ...
    // exit;
        ...
        $comment_update = $this->stage_4_model->where(['pointer_id' => $pointer_id])->set(['preference_comment' => $comment])->update();
