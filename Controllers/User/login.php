...
class login extends BaseController
{
    ...
    public function login_page_backup($redayrect_url = null)
    {
        if (session()->has('user_id')) {
            return redirect()->to(base_url('user/dashboard'));
        }
        ...
        return view('user/login_backup', $data);
    }
    ...
    public function generate_captcha(){
        ...
        $session->set('server_captcha', $captcha_code);
    ...
    public function user_login_check_backup()
    {
    ...
    public function user_login_check()
    {
        ...
        $server_captcha = $session->server_captcha;
        $client_captcha = $this->request->getPost('client_captcha');
        $username = $this->request->getPost('username');
        $where = $this->request->getPost('where');
        $password = bin2hex(trim($this->request->getPost('password')));
        $redirect = base64_decode($this->request->getPost('redayrect_url'));
        

        if (trim($server_captcha) == trim($client_captcha)) {
            // should return JSON with success as true
            if (1==1) {
                $database = $this->user_account_model->where('email', $username)->where('password', $password)->first();
                if ($database) {
    ...
    public function create_an_account_code()
    {
        ...
        $client_captcha = $data['client_captcha'];

        if(trim($server_captcha) != trim($client_captcha)){
            
            $this->session->setFlashdata('error_msg', 'Please check the the captcha form.');
            return redirect()->to(base_url("user/create_an_account"));
            exit;
        }        
    ...
    public function user_Forgot_Password_check()
    {
        ...
        if(trim($server_captcha) != trim($client_captcha)){
            ...
            $this->session->setFlashdata('error_msg', 'Please check the the captcha form.');
            return redirect()->to(base_url("user/forgot_Password"));
            exit;
        }
