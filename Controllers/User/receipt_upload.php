...
class receipt_upload extends BaseController
{
    ...
    public function deleteDocumentAndComments(){
        $comment_id = $this->request->getPost("comment_id");
        $document_id = $this->request->getPost("document_id");
        // echo $document_id;
        // exit;
        $is_deleted = $this->documents_model->delete($document_id);
        if($is_deleted){
            echo $this->comment_store_documents_model->update($comment_id, ["document_id" => 0]);
        }
    }
    ...
    public function receipt_upload_page($ENC_pointer_id)
    {
        ...
        if ($status == "" || $status == "Start") {
            ...
            // 
            $comment_store_documents = $this->comment_store_documents_model->where(["pointer_id" => $pointer_id, "stage" => "stage_2"])->findAll();
    ...
    public function receipt_upload_pagesecond($ENC_pointer_id)
    {
        $pointer_id = pointer_id_decrypt($ENC_pointer_id);
        $stage_3 = $this->stage_3_model->where("pointer_id", $pointer_id)->first();
        // akanksha 7 july 2023
        ...
        if ($status == "" || $status == "Start") {
            ...
            return view('user/stage_3/receipt_upload__aus',$data);
        }
        // return view('user/dashboard');
        return redirect()->to('user/dashboard');
    }
    ...
    //   echo $pointer_id;
    //   die;
    $stage_3_checkin__ = find_one_row('stage_3','pointer_id',$pointer_id);
    ...
    $data__3 = [
        ...
        $this->stage_3_model->set($data__3)->where('pointer_id', $pointer_id)->update();
...
';
 ...
 require_once "dompdf/autoload.inc.php";
    ...
    //  $pointer_id=10;
        ...
        $sss = "public/application/" . $pointer_id . "/stage_3/";
        if (!is_dir($sss)) {
            // create the directory path if it does not exist
            mkdir($sss, 0777, true);
        }
        
        $sss__path = "public/application/" . $pointer_id . "/stage_3";
        $sss = "public/application/" . $pointer_id . "/stage_3/".$pdf_name;
        file_put_contents($sss, $dompdf->output());
   $isAlreadyThere = $this->documents_model->where(["pointer_id" => $pointer_id, "stage" => "stage_3", "required_document_id" => 55])->first();
    if(!$isAlreadyThere){
         $data = array(
                                    'pointer_id' => $pointer_id,
                                    'stage' => 'stage_3',
    ...
    function stage_3_submit_($ENC_pointer_id)
    {
        ...
        if (!empty($mail_temp_4)) {
            ...
            foreach ($comment_store_documents as $comment_store_documents_) {
                ...
                $documents = $this->documents_model->where(['id' => $comment_store_documents_['document_id'], 'stage' => "stage_3"])->first();
    ...
    function submit_pageseconds3($ENC_pointer_id)
    {
        ...
        if (!empty($mail_temp_4)) {
            ...
            if($enroll_stage_4 == false){
                if($preference_location == "Online (Via Zoom)"){
                    $stage_3_Docs_ex = $this->documents_model->where(['pointer_id' => $pointer_id, 'required_document_id' => 55, 'stage' => "stage_3"])->first();
                     $addAttachment_ex = array(
                        [
                            'file_path' => $stage_3_Docs_ex['document_path'] . '/' . $stage_3_Docs_ex['document_name'],
                            'file_name' => $stage_3_Docs_ex['document_name'],
