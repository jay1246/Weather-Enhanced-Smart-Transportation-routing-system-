    ...
    public function stage_1_expired()
    {
        ...
        foreach ($application_pointers as $key => $value) {
            ...
            if (!empty($stage_1)) { // validation check 
                ...
                if (empty($stage_2_add_employment)) { // validation check 
                    ...
                    if (!empty($user_account_)) { // validation check 
                        ...
                        if (!empty($user_email)) { // validation check 
                            ...
                            if ($day_remain == -37) {
                            ...
                            } else if ($day_remain == -29) {
                                if ($archive_email_send != 1) { // validation check 
                                ...
                                if ($expiry_email_send != 1) { // validation check 
...
public function stage_2_expired()
    {
        $application_pointers = $this->application_pointer_model->Where(['stage' => 'stage_2', 'status' => 'Start'])->find();
        foreach ($application_pointers as $key => $value) {
            $pointer_id = $value['id'];
            $stage_1 = $this->stage_1_model->where(['pointer_id' => $pointer_id])->wherein('status', ['Approved', 'Expired'])->first();
            if (!empty($stage_1)) {
                ...
                echo $formattedDate = $dateTime->format('Y-m-d');
        ...
        rmdir($dir);
                ...
                // Open the directory
                 if ($dir_handle = opendir($folder_path)) {
                    ...
                    closedir($dir_handle);
                    rmdir($folder_path);
                     //$document_model=$this->documents_model->where(['pointer_id'=>$pointer_id,'stage'=>'stage_2'])->delete();
                     $document_model = $this->documents_model->where('pointer_id', $pointer_id)->where('stage','stage_2')->delete();
                     $data=[
                         'stage'=>'stage_1',
                         'status'=>'Approved'
                         ];
                     $update_stage=$this->application_pointer_model->where('id',$pointer_id)->set($data)->update();
                     //$update_stage = $this->application_pointer_model->where('id', $pointer_id)->set($data)->update();
...
foreach ($messageIds as $messageId) {
            ...
            if(isset($header->from[0]->mailbox)){
                ...
                if(isset($header->sender[0]->mailbox)){
                    ...
                    if(strpos($header->subject, "Employment Verification") !== false){
                                                ...
                                                if ($check > 1) {
                                                    ...
                                                    if ($total_employ == $is_verification_check) {
                                                        ...
                                                        if (!empty($mail_temp_4)) { // validation check 
                                                            ...
                                                            $isSend = verification_(env('verification_SERVER_EMAIL'), env('verification_SERVER_FROM_EMAIL'), $to, $subject, $message);
...
// }
                                                ...
                                                if ($check > 1) {
                                                    ...
                                                    if ($total_employ == $is_verification_check) {
                                                        ...
                                                        if (!empty($mail_temp_4)) { // validation check 
                                                            ...
                                                            $check = 0;
                                                            $to =  env('ADMIN_EMAIL');
                                                            verification_(env('verification_SERVER_EMAIL'), env('verification_SERVER_FROM_EMAIL'), $to, $subject, $message);
                                                        }
                                                    } else {
    ...
    //code by rohit 
     ...
     //echo "check_email_reminder";
            ...
            //  $email_verification = $this->email_verification_model->where(['email_reminder_date !=' => '0000-00-00 00:00:00.000000','is_verification_done' => 0, 'verification_email_send' => 1])
            //  ->groupBy("pointer_id")->findAll();
          $stage2_data = $this->stage_2_model->where(['status' => 'Submitted'])->findAll();
...
//              die;
              ...
              $email_reminder_datta= $this->email_verification_model->where(['email_reminder_date !=' => NULL,'is_verification_done' => 0, 'verification_email_send' => 1])
             ...
             if($email_reminder_datta['verification_email_send'] == 1 && $email_reminder_datta['is_verification_done'] == 0){
                ...
                if ($stage2_item["email_status_cronjob"] !=15) {
                 $check = verification_(env('SERVER_EMAIL'), env('verification_SERVER_FROM_EMAIL'), $to, $mail_subject, $mail_body); 
   ...
   // echo "hello";
        ...
        ->findAll();
         foreach ($stage_3_interview as $stage_3_interview__){
          ...
          $allowedLocationIds = [1, 2, 3, 4, 17, 11,9];
