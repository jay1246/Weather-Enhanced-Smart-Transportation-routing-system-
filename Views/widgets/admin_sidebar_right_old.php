...
<style>
 ...
 <!--<textarea class="form-control form-control-lg p-1 scroll_chat" id="chatbox" max_rows="5" placeholder="Type message" style="font-size: 18px; width: 94%; margin-left: 1px;"></textarea> -->
  ...
  <div class="m-1" style="display:none;">
             ...
             <input type="file" id="chat_file" name="chat_file[]" value="" class="form-control form-control-lg" multiple accept=".mp3, .mp4, .docs, .pdf, .jpeg, .png, .jpg, .doc, .docx, .xls, .xlsx, .xlsm, .xlsb, .xltx, .xltm">
  ...
  success: function(response) {
    if(response){
        ...
        msg = `<b style="color: ${response.color}; font-size: 13px;">${username}</b> <br> ${truncatedMessage}`;
         ...
         truncatedMessage = response.documents.length > 30 ? response.documents.substring(0,30) + '...' : response.documents;
         msg=`<b style="color: ${response.color}; font-size: 13px;">${username}</b> <br><span style="font-size:19px;">${check_icon_chat(getFileExtension(response.documents))}</span><span style="cursor:pointer;" data-toggle="tooltip" data-placement="top" title="${response.documents}">${truncatedMessage}</span>`;
        }
     ...
     //map global var
      ...
      reply_documets=truncatedMessage;
      reply_documets_path=response.documents_path
      //alert(reply_color);
      console.log(reply_documets + reply_documets_path);
                 ...
                 // console.log(path);
                   popup=`right: 64px;top: -10px;padding: 11px 3px 0px 8px;`;
                    var doc_name_show = item.documents.length > 46 ? item.documents.substring(0,46) + '...' : item.documents;
                     ...
                     //console.log(item);
                    if(item.documents.endsWith('.png') || item.documents.endsWith('.jpg') || item.documents.endsWith('.jpeg')){
                                            //for same user
                   var paragraph = `
        <div id="target${item.id}" class="message-container" style="margin: -1px 0px 6px 32px; max-height: 150px; max-width: 91%; min-width: 91%;"onmouseout="hideReplyIcon(this)" onmouseover="showReplyIcon(this)">
           
            <div id="text_area${item.id}" class="d-flex position-relative justify-content-end">
            <i onclick="reply_msg(${item.id});" class="bi bi-reply-fill  custom_style_icon" style="display:none;"></i>
                <div id="docs_reply${item.id}" class="d-flex align-items-center" style="padding-top: 5px; position: relative; ">
                      <a data-toggle="tooltip" data-placement="top" title="${item.documents}" href="${path}"  target="_blank" >
                            <img src="${path}" width="191px" height="145px" style="border: 2px solid #c8c8c8;border-radius: 10px;"></a>
        <i class="ms-2" style="float: right; font-size: 10px; position: absolute; right: 7px; bottom: 2px;color:black;">${formatTimestamp(item.created_at)}</i>
...
//for diffrent uset
                   var paragraph_ = `
                    ...
                    <div id="target${item.id}" class="message-container" style="margin: -9px 0px 11px -14px; max-height: 150px; max-width: 91%; min-width: 91%;"onmouseout="hideReplyIcon(this)" onmouseover="showReplyIcon(this)">
  ...
  `; 
                  ...
                  var path = base_url+ "/" + lastItem.documents_path;
                   popup=`right: 64px;top: -10px;padding: 11px 3px 0px 8px;`;
                    ...
                    if(lastItem.documents.endsWith('.png') || lastItem.documents.endsWith('.jpg') || lastItem.documents.endsWith('.jpeg')){
              //for same user  for image
                   var paragraph = `
        <div id="target${lastItem.id}" class="message-container" style="margin: -1px 0px 6px 32px; max-height: 150px; max-width: 91%; min-width: 91%;" onmouseout="hideReplyIcon(this)" onmouseover="showReplyIcon(this)">
            <div id="text_area${lastItem.id}" class="d-flex position-relative justify-content-end">
            <i class="bi bi-reply-fill custom_style_icon" style="display:none;"></i>
                <div id="docs_reply${lastItem.id}" class="d-flex align-items-center" style="padding-top: 5px; position: relative; ">
...
`;
        ...
        console.log(base_url);
                   var paragraph_ = `
                    ...
                    <div id="target${lastItem.id}" class="message-container" style="margin: -9px 0px 11px -14px; max-height: 150px; max-width: 91%; min-width: 91%;" onmouseout="hideReplyIcon(this)" onmouseover="showReplyIcon(this)">
                ...
                <div id="text_area${lastItem.id}" class="d-flex position-relative justify-content-end">
                <i class="bi bi-reply-fill custom_style_icon" style="display:none;position: absolute;left: 0px;top: -20px;"></i>
                ...
                `; 
                 ...
                 var paragraph_ = `
                      <div id="target${lastItem.id}" class="message-container" style="margin: 5px 26px 8px -14px; max-height: 97px; max-width: 91%; min-width: 91%;" onmouseout="hideReplyIcon(this)" onmouseover="showReplyIcon(this)">
                <div id="text_area${lastItem.id}" class="d-flex position-relative justify-content-start">
                    <i class="bi bi-reply-fill custom_style_icon" style="display:none;"></i>
                    <img   src="${base_url}${lastItem.profileimg_path}"alt="avatar 1" style="width: 30px;height: 30px;margin-top: 6px; border-radius: 25px;"> 
                    <div id="docs_reply${lastItem.id}" class="d-flex align-items-center" style="border: 2px solid #d9ecd6; border-radius: 10px; padding-left: 6px; padding-top: 5px; background: #d9fdd3; position: relative; width: 300px;">
                        ...
                        <p data-toggle="tooltip" data-placement="top" title="${lastItem.documents}" class="d-inline font-weight-bold mr-3" style="width: auto; width: 84%; margin-top: 9px; margin-left: 6px; overflow: hidden; text-overflow: ellipsis; padding-right: 5px;">
                            <a href="${path}" target="_blank">${doc_name_show}</a><br>
                            <i class="ms-2" style="float: right; font-size: 10px; position:absolute;top: -23px;right: -28px;">${formatTimestamp(lastItem.created_at)}</i>
                        </p>
                        <span class="d-inline" style="font-size: 28px; margin-top: -8px; margin-right: 9px; color: #918b8b;">
                            <a href="${path}" download="${lastItem.documents}">
                                <i class="bi bi-arrow-down-circle"></i>
...
function allowDrop_chat(event) {
 ...
 function handleDrop_chat(event) {
    ...
    event.preventDefault();
        ...
        var allowedTypes = ["mp3","mp4","docs","pdf","jpeg", "png", "jpg","doc","docx","xls","xlsx","xlsm", "xlsb", "xltx", "xltm"];
...
document.getElementById('chat_file').addEventListener('change', function() {
    ...
    if (file) {
        $('#show_img_chat').text(file.name); 
        $('#chat_file_val').val(file.name); 
    }
    ...
    $.ajax({
        ...
        xhr.upload.addEventListener('progress', function(e) {
            if (e.lengthComputable) {
                ...
                $('#progress_chat_bar_').width(percentComplete + '%');
                $('#progress_chat_bar_').text(percentComplete + '%');
                $('#progress_chat_bar_').attr('aria-valuenow', percentComplete);
            }
        }, false);
        return xhr;
    },
        success: function(response) {
           // console.log(response.lastinsertedids_chat);
           // return;
            if(response.data){
                hide_show_docs();
...
function check_icon_chat(icon_type) {
   ...
   // console.log(icon_type);
    ...
    } else if(icon_type == 'png' || icon_type == 'jpg' || icon_type == 'jpeg'){
...
function check_icon_image(icon_type) {
    var icon = "";
    if (icon_type === 'png' || icon_type === 'jpg' || icon_type === 'jpeg') {
        icon = '<i style="color:#6eab7f;"  class="bi bi-file-image"></i>';     
    }
    return icon;
}
