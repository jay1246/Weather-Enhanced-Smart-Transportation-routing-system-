...
// $upload_form = "";
 ...
 ?>
            ...
            <div id="comment_area_<?= $comment_store_document["id"] ?>" style="margin-top: 10px;">
               ...
               if (user_type == 'Applicant') {
                ...
                $("#AJDSAKAJLD").click(function() {
                    ...
                    if (custom_alert_popup_close('AJDSAKAJLD')) {
                        $('#cover-spin').show(0);
                         $.ajax({
                            'url': '<?= base_url('user/stage_3/submit_pageseconds3/' . $ENC_pointer_id) ?>',
                            'type': 'post',
                            success: function(data) {
                                    $('#cover-spin').hide(0);
...
<script>
 function storepdf() {
    ...
    $.ajax({
        url: '<?= base_url('user/stage_3/storepdf') ?>/' + pointerid,
        type: 'GET',
