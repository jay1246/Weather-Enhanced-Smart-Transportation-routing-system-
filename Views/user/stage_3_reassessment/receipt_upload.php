...
<div class="text-center" style="padding: 30px;">
    <a onclick="save_data()" id="cancel" class="btn btn_yellow_green back_btn_id">Back</a>
    <a onclick="save_data()" type="submit" class="btn btn_green_yellow" id="action_validation_btn">Save & Exit</a>
...
<script>
    ...
    //         $stage_4_javscript = true;
    //         }
    //     }
          ?>  
    var stage_4_javscript = '<?=$enroll_stage_4?>';
    function save_data(){
        $('#cover-spin').show(0);
        $.ajax({
            'url': '<?= base_url('user/stage_3_reassessment/save_pointer/' . $ENC_pointer_id) ?>',
            'type': 'post',
             success: function(data) {
                $('#cover-spin').hide(0);
                if(stage_4_javscript){
                    window.location = "<?= base_url('user/view_application/' . $ENC_pointer_id) ?>";
                }else{
                    window.location = "<?= base_url('user/Finish_application/' . $ENC_pointer_id) ?>";
                }
            }
    ...
    * The applicant is currently away, kindly schedule the interview after DD/MM/YYYY.`);
    ...
    function validateForm() {
        ...
        //  working
                ...
                custom_alert_popup_show(header = '', body_msg = "Are you sure you want to submit Stage 3 (Reassessment) Application?", close_btn_text = 'No', close_btn_css = 'btn-danger', other_btn = true, other_btn_name = 'Yes', other_btn_class = 'btn_green_yellow', other_btn_id = 'AJDSAKAJLD');
                // check Btn click
                $("#AJDSAKAJLD").click(function() {
                    ...
                    if (custom_alert_popup_close('AJDSAKAJLD')) {
                        $('#cover-spin').show(0);
                         $.ajax({
                            'url': '<?= base_url('user/stage_3_reassessment/submit_/' . $ENC_pointer_id) ?>',
                            'type': 'post',
                            success: function(data) {
                                    $('#cover-spin').hide(0);
                                    window.location = "<?= base_url('user/view_application/' . $ENC_pointer_id) ?>";
                                            ...
                                            // window.location = "<?= base_url('user/stage_3_reassessment/submit_/' . $ENC_pointer_id) ?>";
